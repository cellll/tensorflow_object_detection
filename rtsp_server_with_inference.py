...
import os
import sys
sys.path.append('/root/object_detection/yolo2')
import yolov2
...
def inference():
    ...
    # arlo web cam init
    file_path = '/tmp/arlo/.arlo'
    lock_path = '/tmp/arlo/.arlo.lock'
    
    lock = FileLock(lock_path, timeout=1000)
    lock.acquire()
    
    num_camera = int(os.environ['NUM_CAMERA'])
    print ('*** CAM {} :: START INITIAL CONNECT'.format(num_camera))
    
    
    USERNAME=os.environ['ARLO_USER']
    PASSWORD=os.environ['ARLO_PW']
    arlo1 = Arlo(USERNAME, PASSWORD)

    cameras1 = arlo1.GetDevices('camera')
    basestations1 = arlo1.GetDevices('basestation')

    url1 = arlo1.StartStream(basestations1[0], cameras1[num_camera-1])
    print ('*** CAM {} :: GOT RTSP URL'.format(url1))

    
    # yolo model initilizing
    yolo = yolov2.YOLOV2_XIILAB()
    print ('*** CAM {} :: DETECTION MODEL LOADED'.format(num_camera))
           
    width = int(os.environ['WIDTH'])
    height = int(os.environ['HEIGHT'])

    # get arlo rtsp stream 
    ...
    while True:
        ...
        if lock.is_locked:
            time.sleep(2)
            lock.release()
            print ('*** CAM {} :: CONNECTION COMPLETE'.format(num_camera))
                  
        ret, frame = cap.read()
        frame = cv2.resize(frame, (width, height))
        #frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        yolo.append_to_list(frame)
        count += 1

        # 30프레임 모이면 한꺼번에 inference후 결과 리스트에 추가 (client가 연결되었을 때만)
        if count == 30:
            result = yolo.inference()

            if shared_rtsp_init.value == True:
                for item in result:
                    shared_result_img_list.append(item)
